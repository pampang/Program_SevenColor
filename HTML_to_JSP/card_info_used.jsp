...
<body>
	...
	<div class="container">
		...
		<div class="col-md-6">
			...
			<div class="col-md-3">
				<a href="/MSCMS/usedListAction.action" class="btn btn-success btn-block active">已发</a>
			</div>
			<div class="col-md-3">
				<a href="/MSCMS/unusedListAction.action" class="btn btn-default btn-block">未发</a>
			</div>
		</div>
		...
		<table class="table table-striped table-bordered">
			<thead>
				<tr>
					...
					<th class="text-center">发卡时间<a href="javascript:;"><span class="glyphicon glyphicon-resize-vertical"></span></a></th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${ pagination.resultList}" var="cardRegisterRecord">
					<tr>
						...
						<td class="text-center">${cardRegisterRecord.card.member.name } </td>
						<td class="text-center">${cardRegisterRecord.dateMemberReceived }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		...
		<div class="btn-group form-group btn-group-sm center">
			<a href="/MSCMS/usedListAction.action?pagination.pageNum=1" class="btn btn-default"><span class="glyphicon glyphicon-fast-backward"></span></a>
			<c:forEach var="pageNum" begin="1" end="${ pagination.pageCount}" > 
				<a href="/MSCMS/usedListAction.action?pagination.pageNum=${pageNum}" class="btn btn-default">${pageNum}</a>
			</c:forEach>
			<a href="/MSCMS/usedListAction.action?pagination.pageNum=${ pagination.pageCount}" class="btn btn-default"><span class="glyphicon glyphicon-fast-forward"></span></a>
		</div>
