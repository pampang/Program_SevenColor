...
<body>
	...
	<div class="container">
		...
		<form id="request-form" class="form-horizontal" role="form" method="post" action="/MSCMS/rechargeStatisticAction.action">
				...
				<div class="form-group">
					<div class="col-md-offset-2 col-md-6">
						<div class="form-group">
							...
							<div class="col-md-8">
								<input id="cardNum" value="${card.cardNumber }" name="card.cardNumber" class="form-control" type="text" placeholder="请输入所统计会员，不输入则统计所有会员">
							</div>
						</div>
		...
		<table class="table table-striped table-bordered">
			<thead>
				<tr class="text-center">
					...
					<th class="text-center">充值时间<a href="javascript:;"><span class="glyphicon glyphicon-resize-vertical"></span></a></th>
					<th class="text-center">充值金额<a href="javascript:;"><span class="glyphicon glyphicon-resize-vertical"></span></a></th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${ pagination.resultList}" var="recharge">
					<tr>
						<td class="text-center">${recharge.card.cardNumber }</td>
						<td class="text-center">${recharge.card.member.name }</td>
						<td class="text-center">${recharge.dateRecharged } </td>
						<td class="text-center money">${recharge.amount }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		...
		<div class="btn-group form-group btn-group-sm center">
			<a href="/MSCMS/rechargeHistoryAction.action?card.cardNumber=${card.cardNumber}&pagination.pageNum=1&beginDate=<fmt:formatDate value="${beginDate}" type="date"/>&endDate=<fmt:formatDate value="${endDate}" type="date" />" class="btn btn-default"><span class="glyphicon glyphicon-fast-backward"></span></a>
			<c:forEach var="pageNum" begin="1" end="${ pagination.pageCount}" > 
				<a href="/MSCMS/rechargeHistoryAction.action?card.cardNumber=${card.cardNumber}&pagination.pageNum=${pageNum}&beginDate=<fmt:formatDate value="${beginDate}" type="date"/>&endDate=<fmt:formatDate value="${endDate}" type="date" />" class="btn btn-default">${pageNum}</a>
			</c:forEach>
			<a href="/MSCMS/rechargeHistoryAction.action?card.cardNumber=${card.cardNumber}&pagination.pageNum=${ pagination.pageCount}&beginDate=<fmt:formatDate value="${beginDate}" type="date"/>&endDate=<fmt:formatDate value="${endDate}" type="date" />" class="btn btn-default"><span class="glyphicon glyphicon-fast-forward"></span></a>
		</div>
	...
	/* 判断会员卡号是否被输入，并转变form的action */
		...
		form.action = document.getElementById("cardNum").value == "" ? "rechargeStatisticAction.action" : "rechargeHistoryAction.action";
		document.getElementById("cardNum").onchange = function(){
			form.action = this.value == "" ? "rechargeStatisticAction.action" : "rechargeHistoryAction.action";
		}
	</script>
